...
function NodeIcon({ type }) {
  if (type === 'scrap') {
    return (
      <svg viewBox="0 0 24 24" className="h-11 w-11 text-accent" fill="none" stroke="currentColor" strokeWidth="1.5">
  ...
  return (
    <svg viewBox="0 0 24 24" className="h-11 w-11 text-accent" fill="none" stroke="currentColor" strokeWidth="1.7">
...
export default function PipelineStep({ item, circleRef, stepRef }) {
  return (
    <div ref={stepRef} className="flex w-full min-w-0 flex-col items-center text-center">
      ...
      <p className="mt-3 w-full max-w-[12rem] break-words text-[1.35rem] leading-tight text-text md:text-[1.55rem]">
        {item.title}
      </p>
      {item.subtitle ? <p className="mt-1 w-full max-w-[12rem] break-words text-[0.9rem] text-muted">{item.subtitle}</p> : null}
